...
**14:06 — บิลจริงใบแรกถึงผู้ปกครองผ่าน OA** เส้นทางเต็มทำงานบนโปรดักชันครั้งแรก (สร้างรหัส 13:55 → ผูก → ปิดยอด → ส่ง OA → โทรศัพท์ได้ข้อความ + ลิงก์เอกสารเข้ารหัส) · ฐานจริง outbox `sent`=1 · **บนเวทีพูดได้แล้วว่า "ทดสอบส่งจริงผ่าน OA แล้ว"** · ข้อสังเกต J-37: บิลเหมาเดือนที่ปิดก่อนมีคาบขึ้น "(เรียนครบ 0 ครั้ง)"

**14:30 — หลังบิลจริงใบแรก** ปุ่ม "คัดลอกข้อความเชิญผู้ปกครอง" (ลิงก์แอด + รหัสในข้อความเดียว น้ำเสียงครู) · ข้อความบิลมีบรรทัดพร้อมเพย์เมื่อเป็นเบอร์โทร (เลขบัตรไม่หลุด) ลิงก์คงไว้บรรทัดท้าย · ตัด "(เรียนครบ 0 ครั้ง)" · พรีวิวลิงก์ใน LINE ที่เป็นภาพเก่าคือแคชของ LINE — เว็บถูกแล้ว แชร์ด้วย `?c=line` ได้พรีวิวใหม่ (J-38–J-40)

**15:00 — ปุ่มส่งเหลือปุ่มเดียว "ส่งใน LINE"** (ผูก OA แล้วส่งผ่าน OA เอง ยังไม่ผูกเปิดแอป LINE) · ไอคอนแท็บเป็นเพนกวินของเจ้าของทุกเบราว์เซอร์ (favicon.svg ฝัง PNG) · หน้าขายมี "เพิ่มเพื่อน LINE" ที่ footer · README ใช้ภาพสี่เหลี่ยม 360px · handoff ฉบับละเอียดสำหรับทีม: `~/Downloads/solo_tutor_handoff_kub.md` (J-41–J-43)
...
## อัปเดต 9 ก.ย. บ่าย–เย็น — ปุ่มเดียว "ส่งใน LINE" · README ใหม่ · บั๊กที่ CI จับได้ (J-41–J-44)

**HEAD `3237b5f`** · เทสในเครื่อง 752 unit / 96 ไฟล์ · 161 e2e · 30 LINE-mock · 48 edge · SQL PG16+17 · CI ต้องเขียวที่คอมมิตนี้ (ดู Actions)

**ปุ่มส่งเหลือปุ่มเดียว "ส่งใน LINE"** (J-41) — ลอง OA ก่อน · OA ไม่ใช่ทางของผู้รับคนนี้ (ยังไม่ผูก/ไม่มีบัญชี/ไม่มี workspace) → เปิดแอป LINE ให้ครูส่งเองอัตโนมัติ · เผยแพร่ลิงก์ล้ม → ไม่ส่งทางไหน · ลำดับ ส่ง/แก้/ข้าม · หลังส่ง OA ปุ่มเป็น "ตรวจสอบผลส่ง" · `oaAvailable()` เป็นจริงทุกข้อความในโหมดจริง จึงตัดสินจากผล `deliveryTarget` เท่านั้น

**บั๊กที่ CI จับได้แต่เครื่องผมมองข้าม** (J-44) — ตัวกัน push อ่านแค่ `tail -2` ของ e2e:mock จึงไม่เห็น "2 failed" · บั๊กจริง: หลังส่ง OA แล้ว timeout การ์ดร่างไม่มีปุ่มตรวจสอบผล/ยกเลิก และข้อความแจ้งผลหาย เพราะ `LineMessageAction` ถูก render คนละตำแหน่งก่อน/หลัง marker → mount ใหม่ · แก้ให้อยู่ที่เดิมตลอด · ตัวกัน push เก็บ output ทั้งก้อนแล้ว grep

**ไอคอนแท็บ** (J-42) — `favicon.svg` ไม่ได้อยู่ในคอมมิตเปลี่ยนโลโก้ Safari เลือก SVG ก่อน จึงยังเห็นเพนกวินเก่า → ฝัง PNG ของภาพเจ้าของใน SVG · **footer หน้าขายมี "เพิ่มเพื่อน LINE"** (J-43) · **พรีวิวลิงก์ใน LINE ยังเก่า = แคชของ LINE** แชร์ด้วย `?c=line`/`?c=pitch`

**README เขียนใหม่ทั้งฉบับ** (`/craft-readme`) — ภาษาอังกฤษ ป้ายปุ่มไทยตามจริง ภาพหน้าจอจริง 6 ภาพใน `docs/images/` (จาก `scripts/mobile-check.mjs`) ทุกตัวเลข/ลิงก์/คำสั่งตรวจกับดิสก์ก่อนคอมมิต · `docs/features.md` ตัดคำว่า "ส่งด้วย LINE OA" ออกแล้ว

**เอกสารส่งต่อสำหรับทีม**: `~/Downloads/solo_tutor_handoff_kub.md` (ภาษาไทย ทุกปุ่ม ทุกขั้นตอน) และสำเนาในชุดพิทช์ทั้งสองชุด

## อัปเดต 12 ก.ย. — ปฏิทินเป็นหน้าหลัก · ล็อกคิว · ตัวนับคอร์ส · เอาคำอธิบายออกจากหน้าจอ (`3bd9b99`)

**ทำอะไร** (รายละเอียดต่อข้อ: `docs/production-ledger.md` J-51…J-57)

1. **ปฏิทินอยู่บนสุดของหน้าแรก** กดวันไหนก็เห็นว่าใครเรียนกี่โมง · วันข้างหน้าเช็คชื่อไม่ได้ เหลือแต่ "เลื่อน" · ค่าเริ่มต้นยังเป็นวันนี้เสมอ
2. **จองล่วงหน้าเป็นชุด** — "ทุกจันทร์+พฤหัส 17:00 อีก 8 สัปดาห์" ในคำสั่งเดียว ปุ่มบอกจำนวนคาบที่จะได้จริงก่อนกด
3. **ล็อกคิว** — เวลาที่เป็นของนักเรียนคนอื่นถูกกันไว้พร้อมบอกชื่อ จองทับต้องกด "สอนกลุ่ม" เอง (ทั้งตอนจองและตอนเลื่อน)
4. **"สอนไปแล้ว 3/10"** บนแถวของวัน หน้ารายชื่อ และหน้านักเรียน · ครูตั้งค่าเริ่มต้นของตัวเองได้ใน `⋯` → จำนวนครั้งต่อคอร์ส และตั้งรายคนทับได้ · แพ็กใช้ตัวนับเดิมของแพ็ก
5. **ปุ่มส่งปุ่มเดียวต่อการ์ด** ในค้างจ่ายและการบ้าน (เดิมมีสองปุ่มชื่อเดียวกัน) · การ์ดแชร์ไม่ขึ้นในเดือนที่ยังเป็น 0
6. **เอาคำอธิบายวิธีใช้ออกจากหน้าทำงาน** ตามที่สั่ง — เก็บเฉพาะคำเตือนที่เปลี่ยนสิ่งที่ครูต้องทำต่อ (พร้อมเพย์ยังไม่ตั้ง · ลิงก์ปิดไม่ได้ · ยังไม่สำรองข้อมูล · เหตุผลที่ปุ่มเช็คชื่อหาย · คิวชนกับใคร)

**หลักฐาน** unit 858 · e2e 161 · mock OA 38 — สองข้อใหม่ในชุด mock พิสูจน์ว่า **การบ้าน** และ **คำตอบในแชท**
ออกทางเดียวกับบิลจริงผ่าน LINE OA (เดิมมีแต่บิลกับใบเตือน) · `node scripts/qa-walk.mjs` เดินทั้งแอปที่ขนาด iPhone 13 แล้วสะอาด

**ยังไม่พิสูจน์กับของจริง** ข้อ 1–5 ยังไม่เคยถูกกดบนมือถือจริง — ขั้นตอน 15 นาทีอยู่ที่ `docs/owner-setup.md` ข้อ 3c
การส่งผ่าน OA ที่พิสูจน์กับของจริงแล้วมีเฉพาะ **บิล** (9 ก.ย. 14:06) การบ้าน/แชท/ค้างจ่าย ผ่านเฉพาะชุด mock

---

## อัปเดต 12 ก.ย. ค่ำ — ซ้อมเดโม→OA ผ่านแล้วจริง · เช้าวันพิทช์ทำตาม `docs/owner-setup.md` ข้อ 3a

ข้อความจากเดโมถึงมือถือจริงผ่าน OA แล้ว 2 ใบ (ชุดเหมาเดือน · คุณแม่กิ๊ฟ · 21:25) — เป็นการพิสูจน์เส้นทางการบ้าน/แชท/ค้างจ่ายกับ production ครั้งแรก
สิ่งที่เจอระหว่างซ้อมและแก้แล้ว: J-61 (QA รอบเต็ม) · J-62 (บอทเงียบเมื่อผู้ปกครองที่ผูกแล้วพิมพ์รหัส · ล็อกส่งไม่บอกเหตุผล → แก้ `70db644`)
**กับดักเช้าวันพิทช์**: ต้องรีเซ็ตเป็นชุดผสมตอนเช้า (คาบ "วันนี้" ปักตามวันที่รีเซ็ต วันอาทิตย์ไม่มีคาบตามตาราง) · คุณพ่อภูมิต้องเป็น LINE เครื่องที่สาม · ห้ามส่งบิลซ้อม (คีย์กันซ้ำไม่มีวันที่)

## อัปเดต 12 ก.ย. เย็น — เจ้าของลองของจริงแล้วทักกลับสองเรื่อง (`afe232a`)

รายละเอียด: `docs/production-ledger.md` J-59, J-60

1. **ปฏิทินย่อได้** (J-59) — "อันนี้มันรกไป" ตารางเดือนเต็มดันงานของวันนี้ตกจอ โดยเฉพาะบนโน้ตบุ๊ก
   ตอนนี้ย่อเหลือ **สัปดาห์เดียว** เป็นค่าเริ่มต้น มี **ปุ่มปฏิทินเล็ก** กางเป็นทั้งเดือน กดอีกทีย่อกลับ
   (เลือก toggle แทนหน้าแยก เพราะได้พฤติกรรม "กดอีกทีกลับมาที่เดิม" ที่ขอ โดยไม่ต้องมีปุ่มย้อนกลับ)
   ลูกศรเลื่อนทีละสัปดาห์ตอนย่อ ทีละเดือนตอนกาง · ช่องวันสูงคงที่ 48px การ์ดกว้างไม่เกิน 460px
2. **ตัวนับคอร์สที่อ่านออก** (J-60) — "7/10 คืออะไร 8/10 คืออะไร"
   เขียนเต็มว่า **"สอนไปแล้ว 7/10"** ทุกที่ · ครบแล้วขึ้นป้าย **"ครบคอร์สแล้ว"** ตั้งแต่หน้ารายชื่อ
   ในหน้านักเรียนมีปุ่ม **ต่อคอร์ส** (เริ่มนับใหม่ 0/N ประวัติและบิลไม่หาย ถามยืนยันก่อน)
   และ **แถมครั้งให้** (เพิ่มเพดานของรอบนี้ กดซ้ำบวกทบ ล้างเมื่อต่อคอร์สใหม่)

**หลักฐาน** unit 864 · e2e 173 · mock OA 38 · `tsc` สะอาด · CI เขียวที่ `afe232a`
เทสใหม่ `tests/e2e/calendar.spec.ts` ยืนยันข้อกำหนดจริง: แถวแรกของวันนี้ต้องอยู่ในจอแรกโดยไม่ต้องเลื่อน
(วัดได้ y=419px บนจอสูง 900px · กางทั้งเดือนแล้วยังอยู่ที่ 647px)
...
## 7) รอบ 9 ก.ย. เย็น — LINE OA ในมุมครู (ชุด A-safe) และแผน v2

เจ้าของเดินเล่นแอปในมุมครู 16:00–16:20 แล้วส่งภาพหน้าจอมา 5 ข้อ: เดโมส่ง OA จริงไม่ได้ · ไม่มีปุ่มให้ผู้ปกครองแอด OA · ใช้ OA ยากมากในมุมครู · เมนูซ้ำ · ข้อความค้างจ่ายมีลิงก์ที่เปิดได้เฉพาะเครื่องครูแทนที่จะมีพร้อมเพย์ · และสั่ง "วางแผนให้ทุกเมลใช้ OA ได้" แผนอยู่ที่ **`docs/line-oa-v2-plan.md`** (แทน `docs/line-oa-plan.md` 6 ก.ย.) — ledger J-45…J-49

**ทำแล้วรอบนี้ (ไม่แตะฐานข้อมูล) — commit `67091b1` · CI run 34339714529 เขียว (integrations/test/build/deploy) · unit 789 / e2e 161 / mock 32 · รีวิวโดย reviewer แยกเลน: REQUEST_CHANGES 5 ข้อ → แก้ครบ → APPROVE:**
- J-45 ใบเตือนค้างจ่ายลงท้ายด้วยบรรทัดพร้อมเพย์แทน "รายละเอียดที่ <ลิงก์>" — ผูกด้วยเลขบัตรยังคงลิงก์ QR (`payOrLink` ใน `messages.ts`)
- J-46 เมนู `⋯` แยกหมวด ผู้ปกครองและรายชื่อ / สำรองและกู้คืน · เดโมไม่มี "รูปแบบการเก็บเงิน" (ซ้ำกับสลับชุดข้อมูล) · รีเซ็ตย้ายไปแท็บ เดโม
- J-47 ปุ่ม **"เชิญผู้ปกครองเข้า LINE"** บนการ์ดข้อความทั้ง 3 แท็บ + หน้านักเรียน + หน้า LINE OA ผ่าน hook เดียว `useLineLink()` (แคชสถานะร่วม) · หน้า LINE OA มี 3 ขั้นบนสุด ฟอร์ม secret/token พับเมื่อเชื่อมแล้ว

**ยังไม่ทำ (ตามแผน):** ชุด A-demo (J-48 — เดโมส่ง OA จริง, เกณฑ์ผ่าน = มือถือจริงได้รับ) · ชุด B (J-49 — OA กลางของ Solo, ต้อง migration + APPLY + เจ้าของตัดสินใจ 3 เรื่องใน v2-plan §5.3)

**กับดักรอบนี้:**
- สองเลนแก้ต้นไม้เดียวกัน: เปลี่ยน template ใบเตือน (J-45) ทำให้ fixture ของ `shared-document-send.test.tsx` (เลนอื่น) หาร่างที่มีลิงก์ไม่เจอ — fixture ที่พึ่ง "ข้อความชนิดไหนมีลิงก์" ต้องบอกเหตุผลไว้ในเทส
...
## 8) รอบ 9 ก.ย. ค่ำ — ชุด A-demo: โหมดเดโมส่งผ่าน LINE OA จริงได้ (`d910dcc` · CI run 34345106418 เขียว · deploy แล้ว)
...
**Follow-up สามข้อ — ทำแล้วรอบเดียวกัน (9 ก.ย. ค่ำ):**
1. `faq.ts` ตัดด่านโหมดจริงข้อที่เจ็ดออก เดโมตอบทีละใบเหมือนโหมดจริง
2. `useLineLink` มี in-flight map ระดับโมดูล — `refresh()` แบบบังคับจะ **join** คำขอที่เกิดหลัง intent ของผู้เรียก แทนที่จะยิงทับ เปิดหน้าตั้งค่าจึงถามสถานะผู้จ่ายคนละครั้งเดียว (ของเดิมเปลือง 3N ไม่ใช่ 2N) แต่ "ตรวจสถานะอีกครั้ง" ยังยิงใหม่จริงทุกคน
3. เดโมที่มี intent ค้างขึ้นแถบเหลืองบอกเหตุผล (`demoFrozenByPendingOa` + `copy.demoPaused`) แทนที่จะเงียบ

**บั๊กที่เจอระหว่างทาง (สำคัญ):** `refreshDemoDay` เดิม *เดินวันต่อ* ทั้งที่ปฏิเสธการสร้างชุดข้อมูลใหม่ (`{...saved, today: now}`) → เดือนเท่ากันในรอบถัดไป → สมุดเดโมจะ **ไม่มีวันถูกสร้างใหม่เลย** แม้ครูเคลียร์การ์ดแล้ว จนกว่าจะข้ามเดือนอีกรอบ · แก้เป็นคืน `saved` เดิมไม่แตะ ทำให้ทั้งแถบเตือนและการฟื้นตัวรอบถัดไปทำงานถูก (เทสยืนยันการ rebuild หลัง `oaDelivery` หาย)

**รอบแก้ตามรีวิว (ต่อจาก `f41ac71`):** (ก) `intentTicket` ที่เดารู้เจตนาจาก render ถูกลบ — `refresh(only?, {joinInFlight})` join ได้เฉพาะเมื่อสั่ง และมีแต่ effect ตอนเปิดหน้าตั้งค่าที่สั่ง ทุกปุ่มที่ครูกดยิงใหม่เสมอ (ของเดิมกดตรวจสถานะระหว่างคำขอเก่ายังค้าง จะได้คำตอบที่เกิด*ก่อน*ผู้ปกครองพิมพ์รหัส) · (ข) `demoFrozenByPendingOa` → `demoPauseReason(state): 'pendingOa' | 'reopen' | null` แถบเตือนโชว์ตาม "เดือนไม่ตรง" อย่างเดียว ส่วนการ์ดที่ค้างเป็นแค่ตัวเลือกประโยค — ของเดิมพอครูเคลียร์การ์ดเสร็จ แถบหายทันทีทั้งที่ข้อมูลยังค้างเดือนเดิม = ความสับสนแบบเดียวกับที่แถบนี้เกิดมาเพื่อกัน

**กับดักที่รีวิวยืนยัน:** หน้าตั้งค่าเคยยิง `line_delivery_target` **สามเท่า**ของจำนวนผู้จ่ายเมื่อเปิดตรงมาที่ URL (แคชเย็น) เพราะ `claimOwner` เลื่อน generation ซึ่งเป็น dep ของ effect → effect รันซ้ำแล้วยิงอีกชุด ส่วนเข้าจากหน้าแอดมิน (แคชอุ่น) เป็นสองเท่า ตอนนี้เหลือชุดเดียวทั้งสองเส้น · คำขอที่ join ค้างได้ไม่เกินราว 25 วินาที ไม่ใช่ 12 เพราะ `authenticatedFetch` ลองใหม่อีกครั้งเมื่อเจอ 401

**ยังเหลือ:** `clearDemoLineIds` ไม่ bump revision (best-effort โดยตั้งใจ) · หน้าตั้งค่า LINE ยังอ่าน channel ซ้ำตอนเปิด (N+1 ไม่ใช่ N) · module state อ่านตอน render ใน `useLineLink` (ควรเป็น `useSyncExternalStore` ถ้าโตกว่านี้) · เคลียร์การ์ดกลางเซสชันแล้วแถบหายแต่ข้อมูลยังค้างเดือนเดิมจนเปิดแอปใหม่ (ข้อความบอกไว้แล้ว)
