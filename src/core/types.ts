...
export interface Subject {
  ...
  /**
   * จำนวนครั้งที่เช็คชื่อไปแล้วตอนกด "ต่อคอร์ส" — ตัวนับเริ่มใหม่จากจุดนี้
   * เก็บเป็นจำนวน ไม่ใช่วันที่ เพราะ CompletionEvent มีแค่ระดับวัน ต่อคอร์สแล้วเช็คชื่อวันเดียวกันจะแยกไม่ออก
   * ประวัติการเช็คชื่อและบิลเดิมไม่ถูกลบ แค่ไม่นับเข้าคอร์สรอบใหม่
   */
  courseBaseline?: number
  /** ครั้งที่แถมเพิ่มให้ในคอร์สรอบนี้ — บวกเข้ากับจำนวนครั้งของคอร์ส ไม่แตะการคิดเงิน */
  courseBonus?: number
...
/**
 * ใบแจ้งว่าครูสั่งลบใครไปแล้ว — ต้องอยู่ในไฟล์นี้ ไม่ใช่ในโมดูลตรรกะ
 * เพราะ `lineProtocol.ts` ที่ Edge Function ใช้ import ชนิดจากที่นี่ Deno จึงต้องไล่ตามไปถึงทุกไฟล์ที่อ้างถึง
 */
export interface SubjectTombstone {
  /** subjectId ที่ถูกสั่งลบ — คนละคนที่ชื่อซ้ำกันคนละ id จึงไม่ถูกกลืน */
  id: string
  /** วันที่ครูกดลบ (state.today ของเครื่องนั้น) */
  at: ISODate
  /**
   * ผลที่เกิดขึ้นจริงตอนกดลบ
   * removed = ลบทั้งแถวได้เพราะไม่มีประวัติการเงิน · archived = ปิดรายการเพราะมีบิลหรืองานที่ทำแล้ว
   * เก็บไว้เพื่อไม่ให้หลุมศพ "แรงขึ้น" ตอนไปเจอสำเนาที่เก่ากว่า — สิ่งที่ครูปิด ต้องปิด ไม่ใช่ลบทิ้ง
   */
  mode: 'removed' | 'archived'
}

/**
 * ผู้จ่ายที่ถูกลบออกไปพร้อมนักเรียนคนสุดท้ายของเขา
 *
 * ต้องเป็นรายการของตัวเองไม่ใช่เดาจากหลุมศพนักเรียน เพราะสองอย่างนี้ไม่ได้แปลว่ากันและกัน:
 * ผู้จ่ายที่ยังมีลูกอีกคนไม่ได้ถูกลบ และเซิร์ฟเวอร์ฝั่ง LINE ก็ผูกข้อมูลไว้กับ "ผู้จ่าย" ไม่ใช่นักเรียน
 * ใบนี้คือสัญญาณเดียวที่ได้รับอนุญาตให้สั่งลบข้อมูลผู้ปกครองบนเซิร์ฟเวอร์ — ดู migration 0018
 */

export interface ClientTombstone {
  /** clientId ในสมุดบัญชี ซึ่งเซิร์ฟเวอร์เก็บเป็น local_client_key */
  id: string
  at: ISODate
}
...
export interface AppState {
  ...
  /**
   * นักเรียนที่ครูสั่งลบ — เดินทางไปกับไฟล์สำรองและก้อนบนคลาวด์ เพื่อให้สำเนาที่เก่ากว่าพาคนนี้กลับมาไม่ได้
   * ไม่มี = ไม่เคยลบใคร (ไฟล์สำรองและข้อมูลเก่าเปิดได้เหมือนเดิมทุกไบต์) · ดู tombstones.ts
   */
  deletedSubjects?: SubjectTombstone[]
  /**
   * ผู้จ่ายที่ถูกลบไปพร้อมนักเรียนคนสุดท้ายของเขา — สัญญาณเดียวที่สั่งลบข้อมูลผู้ปกครองบนเซิร์ฟเวอร์ได้
   * ไม่มี = ไม่เคยลบผู้จ่ายคนไหน · ดู tombstones.ts และ migration 0018
   */
