...
export async function readSnapshot(signal?: AbortSignal): Promise<CloudSnapshot | null> {
  const rows = await request<Partial<CloudSnapshot>[]>(
    '/rest/v1/ledger_snapshots?select=revision,schema_version,cipher,iv,kdf,updated_at,device&limit=1',
    { signal },
  )
  ...
  return {
    ...
    ...(typeof row.kdf === 'string' ? { kdf: row.kdf } : {}),
  }
}
...
export async function saveSnapshot(input: {
  expected: number; revision: number; schema: number; cipher: string; iv: string; device: string; signal?: AbortSignal
}): Promise<SaveResult> {
...
export const deleteSnapshot = (signal?: AbortSignal): Promise<boolean> => rpc<boolean>('delete_ledger_snapshot', {}, signal)

export async function deleteTeacherAccount(password: string, signal?: AbortSignal): Promise<boolean> {
  const result = await invoke<{ ok?: unknown }>('delete-account', { password, confirmation: 'DELETE' }, signal)
