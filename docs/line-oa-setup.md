...
## ส่งจากแท็บค้างจ่ายและการบ้าน (8 ก.ย. 2569)

หน้าแอดมินมีสองแท็บที่ใช้ปุ่ม **ส่งด้วย LINE OA** ชุดเดียวกับการ์ดข้อความ (โค้ดกลาง `src/app/oaSend.ts`):

- **ค้างจ่าย** — บิลค้างทุกใบเรียงตามวันค้าง แสดงขั้นแจ้งยอด (สุภาพ/ชัดเจน/รอบสุดท้าย) ร่างที่รอส่ง แจ้งยอดล่าสุด และสถานะเชื่อม OA ของผู้ปกครองแต่ละคน · ปุ่ม **ส่งแจ้งยอดทั้งหมดผ่าน LINE OA (n)** ส่งทีละใบตามลำดับที่เห็น ข้ามผู้ปกครองที่ยังไม่เชื่อมและบอกเหตุผล ครูเห็นข้อความทุกใบในรายการก่อนกด (หลักการ "ครูกดส่ง" ยังอยู่ — กดหนึ่งครั้งต่อชุดที่ตรวจแล้ว) · **เตือนยอดสั้น** สร้างข้อความเตือนยอดวันละใบต่อบิลเข้าคิว
- **การบ้าน** — มอบหมายให้หลายคนพร้อมกันพร้อมกำหนดส่ง ระบบร่างข้อความแจ้งผู้ปกครอง เมื่อเลยกำหนดและยังไม่ทำเครื่องหมาย "ได้รับแล้ว" ระบบร่างข้อความเตือนให้เอง เตือนซ้ำได้วันละใบ ทำเครื่องหมายได้รับแล้วร่างเตือนถอนตัวเอง
- ทั้งสองแท็บไม่ยิง LINE ตรง — ทุกใบผ่าน `enqueue_line_message` → `line-send` เหมือนเดิม จึงได้ dedupe/quota/ผลส่งชุดเดียวกัน และการส่งด้วย share-link (เปิด LINE แล้วยืนยันเอง) ยังอยู่ที่แท็บรอส่ง
