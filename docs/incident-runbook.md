...
| `[ops] backup ล้ม` | `backup.yml` รายวัน | dump/เข้ารหัส/ตรวจกลับล้ม → ไม่มีสำเนาของวันนั้น |
...
## 5. ค่าที่ทีมตั้งไว้ (ช่วง pilot)

ค่าเหล่านี้เป็น **ข้อเสนอของทีมเทค** ทีมยืนยันได้ในที่ประชุม แล้วแก้ตารางนี้ให้เป็นข้อตกลง

| หัวข้อ | ค่า | หมายเหตุ |
|---|---|---|
| RTO (กลับมาให้บริการภายใน) | 24 ชม. | หน้าเว็บเป็น static บน GitHub Pages กู้ได้เร็วกว่านั้นมาก ตัวที่กินเวลาคือ Supabase |
| RPO (ยอมเสียข้อมูลฝั่งเซิร์ฟเวอร์ย้อนหลัง) | 24 ชม. | `backup.yml` รันทุกวัน 03:35 น. ไทย · **ข้อมูลหลักของครูอยู่ในเครื่องครู จึงไม่หายตามเซิร์ฟเวอร์** |
| ผู้รับ alert หลัก | เต้ (เจ้าของ repo) | GitHub Issue ป้าย `ops-alert` แจ้งเข้าอีเมล GitHub อยู่แล้ว |
| ผู้รับ alert สำรอง | อีก 1 คนในทีม | ต้อง watch repo แบบ All Activity หรือถูก assign ใน Issue |
| ช่องทาง alert เพิ่ม | LINE กลุ่มทีม | ต้องตั้ง Secret `OPERATIONS_ALERT_WEBHOOK` (ยังไม่ตั้ง — ไม่มีก็ยังได้ Issue) |
| ผู้ถือ `BACKUP_PASSPHRASE` | 2 คน (เต้ + อีก 1) | ห้ามถือคนเดียว ถ้าคนนั้นหายไปคือกู้ backup ไม่ได้เลย |
| หมุน secret | ทุก 6 เดือน หรือทันทีเมื่อสงสัยหลุด | จดวันหมุนใน `docs/data-inventory.md` |
