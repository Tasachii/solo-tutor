...
## สถานะ (อัปเดต 8 ก.ย. เช้า)

**เฟส 0, 2, 3, 4 เสร็จ (ส่วนที่ทำได้จากฝั่งโค้ด) · ช่องว่างที่ปิดแล้ว: 1, 3, 4, 5, 6, 7, 8, 10 · เหลือที่ต้องใช้บัญชีเจ้าของ: 9 (LINE) · โดเมน · SMTP · พร้อมเพย์รับค่าสมาชิก**
...
| T4 เพิ่มหลายคนจากหน้ารายชื่อ (ตัดชื่อซ้ำ ข้ามคนที่มีแล้ว ค่าเริ่มต้นแพ็ก) + แก้บั๊กเดาหัวตารางที่ทำให้นักเรียนคนแรกหาย | unit 8 · e2e 2 (วาง 25 บรรทัดจริง) · mutant 3 ตาย |
| T6 ปุ่ม "คัดลอกข้อความแจ้งเตือน" บนบิลค้าง — คัดลอกอย่างเดียว ไม่แตะสถานะ ตัวเลขจาก ledger คำลงท้ายตามครู | unit 5 · e2e 2 อ่านคลิปบอร์ดจริง · mutant 2 ตาย |
| T2 ไล่เส้นเดโม 6 ขั้น: เช็คชื่อซ้ำ idempotent · ปิดยอดซ้ำไม่สร้างบิลซ้ำ · รับยอดซ้ำไม่ออกใบเสร็จสอง (reducer) · คิวส่งอยู่ใน state รอด iOS ทิ้งแท็บ · ส่งบิลที่ยอดเปลี่ยนถูกกัน (financialRevision) — reducer ไม่พบบั๊ก แต่**เดินบนเว็บจริงเจอ 1 ข้อ**: เดโมเดินตามปฏิทินจริง ทำให้เลขที่บิลของน้องภูมิสุ่มเป็นเคส "สลิป 3,500 เกินบิล 3,000" ซึ่งปุ่มรับยอดถูกปิดไว้ — ครูค้างหน้านั้น แก้ให้รับเท่ายอดบิลและจดยอดสลิปไว้ | อ่านโค้ด + e2e demo-flow 17 ข้อ + เดินเว็บจริง · unit 1 (mutant ปุ่มปิดตาย) |
| **เฟส 3** บัญชีครู + สำรองสมุดบัญชีขึ้นคลาวด์ (เข้ารหัสด้วยรหัสผ่านครูบนเครื่อง PBKDF2→AES-GCM · snapshot เดียวต่อครู · revision ชนแล้วถามครู ไม่ทับเงียบ) · session อยู่ localStorage ไม่ต้องล็อกอินทุกวัน · หน้านโยบายอัปเดต | unit 27 · e2e 2 + QA จำลอง 2 · sql (stale writer ถูกปฏิเสธ · RLS) · migration 0005 บนโปรเจกต์จริง · anon → 401 ทุกทาง |
| **เฟส 4** แพ็กสมาชิก: เพดานฟรี 5 คนที่ยังเรียนอยู่ (เพิ่มทีละคน/หลายคน/onboarding เจอชีทแพ็ก) · ขอเปิด Pro ในแอป (ราคาคิดฝั่งเซิร์ฟเวอร์) · ทีมอนุมัติด้วย SQL (docs/plan-approval.md) · พัก/ใช้ต่อเลื่อนวัน · ใบเสร็จค่าสมาชิก SP-YYYYMM-0001 · วันคิดเป็นวันไทย | unit 6 (mutant เพดาน 6 ตาย) · e2e คนที่ 6 · QA จำลอง 1 · sql 9 เงื่อนไข · migration 0006 บนโปรเจกต์จริง · anon → 404/401 |
| เดินเว็บจริงหลัง deploy (headless): เมนู 11 ปุ่ม · เส้นเดโม · บัญชีจริง: สมัคร → ซิงก์แล้ว → แพ็กฟรี → ลบคลาวด์ · **สองเครื่อง**: A สมัคร+push → B เครื่องใหม่เข้าสู่ระบบแล้วดึงรายชื่อของ A ได้ · เจอบั๊กที่สอง: ทุกครั้งที่เปิดแอป `track('app_open')` ขยับ revision ทำให้เครื่อง A มอง "ตัวเองแก้" → conflict ทั้งที่แค่เปิดดู แก้เป็นลายนิ้วมือของตารางสมุดบัญชี (ตัด events/today ออก) | console error 0 · user ทดสอบลบผ่าน Admin API แล้ว · unit 1 (mutant รวม events ตาย) |
| Backup: secrets ครบแล้ว (`BACKUP_PASSPHRASE`, `SUPABASE_DB_PASSWORD`) รันจริงสำเร็จ artifact เข้ารหัส | workflow run เขียว |

**ยังเปิดอยู่:** 9 (OA จริง — ต้องบัญชี LINE ของเจ้าของ ผมเข้า LINE Developers ให้ไม่ได้เพราะต้องล็อกอิน LINE) · `SOLO_PROMPTPAY` ว่าง (QR รับค่าสมาชิกยังไม่ขึ้น) · Resend/ลืมรหัสผ่าน · โดเมน · 11–13
...
| # | ระดับ | เรื่อง | แก้เฟส |
|---|---|---|---|
| 1 | ~~blocker~~ ปิดแล้ว | ข้อมูลครูอยู่ localStorage เครื่องเดียว → เฟส 3 ซิงก์เข้ารหัสขึ้นคลาวด์ | 3 ✓ |
| 2 | blocker | Supabase Free "paused after 1 week of inactivity" | 2 |
| 3 | ~~blocker~~ ปิดแล้ว | Free ไม่มี automatic backup → workflow รายสัปดาห์ เข้ารหัส เก็บ 90 วัน | 2 ✓ |
...
| 8 | ~~medium~~ ปิดแล้ว | สัญญาหน้าราคา: เพดาน 5 · plan · พัก · ขอเปิด Pro + ใบเสร็จ — เหลือใส่พร้อมเพย์รับเงินของเรา | 4 ✓ |
